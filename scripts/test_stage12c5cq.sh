...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cp.sh" -- "$@"
fi

printf '\n== external process kind elimination ==\n'
for test_name in prompt_interaction legacy_reta_prompt table_adapters; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cq"
    printf '== run test_%s_12c5cq ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cq"
done
...
printf '%s\n' 'stage12c5cq external process kind elimination complete'
