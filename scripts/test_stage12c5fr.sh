...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5fq.sh" -- "$@"
fi

printf '\n== shared library architecture copyability fix ==\n'
# Explicit source names kept for guards: tests/test_shared_library_architecture.mojo test_shared_library_architecture_12c5fr
for test_name in shared_library_architecture; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5fr"
    printf '== run test_%s_12c5fr ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5fr"
done
...
printf '%s\n' 'stage12c5fr shared library architecture copyability fix complete'
