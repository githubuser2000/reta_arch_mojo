...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5ca.sh" -- "$@"
fi

printf '\n== stored delete dispatch ownership ==\n'
for test_name in prompt_interaction legacy_reta_prompt table_adapters; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cb"
    printf '== run test_%s_12c5cb ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cb"
done

"$ROOT/scripts/run_pytest.sh" -q \
    tests/test_stage12c5cb_source.py \
    tests/test_stage12c5ca_source.py \
    tests/test_stage12c5bz_source.py \
...
printf '%s\n' 'stage12c5cb stored delete dispatch ownership complete'
