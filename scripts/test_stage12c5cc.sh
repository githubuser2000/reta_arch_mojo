...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cb.sh" -- "$@"
fi

printf '\n== table adapter counting parity boundary ==\n'
for test_name in table_adapters prompt_interaction legacy_reta_prompt; do
...
printf '%s\n' 'stage12c5cc table adapter counting parity boundary complete'
