...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5df.sh" -- "$@"
fi

printf '\n== prompt runtime shell split owner ==\n'
for test_name in prompt_interaction prompt_external_commands legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5dg"
    printf '== run test_%s_12c5dg ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5dg"
done
...
printf '%s\n' 'stage12c5dg prompt runtime shell split owner complete'
