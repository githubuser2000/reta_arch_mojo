...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cx.sh" -- "$@"
fi

printf '\n== fallback interaction argv plan ==\n'
for test_name in prompt_interaction legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cy"
    printf '== run test_%s_12c5cy ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cy"
done
...
printf '%s\n' 'stage12c5cy fallback interaction argv plan complete'
