...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5bm.sh" -- "$@"
fi

printf '\n== frozen combined classic/property/catalog outer order ==\n'
PYTHONHASHSEED=0 PYTHONDONTWRITEBYTECODE=1 \
    "$TEST_PYTHON" scripts/check_prompt_combined_outer_order_reference.py

printf '\n== compile/run complete combined multi-domain fraction probe ==\n'
"$ROOT/scripts/check_prompt_true_fraction_multiples.sh" -- "$@"

"$ROOT/scripts/run_pytest.sh" -q \
    tests/test_stage12c5bn_source.py \
    tests/test_prompt_combined_outer_order_source.py \
...
printf '%s\n' \
    'stage12c5bn combined classic/property/catalog outer axes complete'
