...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cu.sh" -- "$@"
fi

printf '
== prompt fallback argument ownership ==
'
...
"$ROOT/scripts/run_pytest.sh" -q \
    ...
    tests/test_stage12c5cs_source.py \
...
printf '%s
' 'stage12c5cv prompt fallback argument ownership complete'
