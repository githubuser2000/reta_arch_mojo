...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5dc.sh" -- "$@"
fi

printf '\n== fallback process run flag ==\n'
for test_name in prompt_interaction legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5dd"
    printf '== run test_%s_12c5dd ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5dd"
done
...
printf '%s\n' 'stage12c5dd fallback process run flag complete'
