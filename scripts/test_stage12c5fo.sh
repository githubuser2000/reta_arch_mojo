...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5fn.sh" -- "$@"
fi

printf '\n== shared library target architecture ==\n'
"$ROOT/scripts/build_shared_library_targets.sh" --dry-run -- "$@"
for test_name in shared_library_architecture architecture_facade legacy_reta_program prompt_execution; do
    printf '\n== build tests/test_%s.mojo ==\n' "$test_name"
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5fo"
    printf '== run test_%s_12c5fo ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5fo"
done
...
printf '%s\n' 'stage12c5fo shared library target architecture complete'
