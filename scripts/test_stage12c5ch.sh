...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cg.sh" -- "$@"
fi

printf '\n== bare deterministic prompt output dispatch ownership ==\n'
for test_name in prompt_interaction legacy_reta_prompt table_adapters; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5ch"
    printf '== run test_%s_12c5ch ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5ch"
done
...
printf '%s\n' 'stage12c5ch bare deterministic prompt output dispatch ownership complete'
