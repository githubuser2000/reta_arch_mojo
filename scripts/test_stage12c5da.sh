...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cz.sh" -- "$@"
fi

printf '\n== prompt profile explicit copy ==\n'
for test_name in prompt_interaction; do
    printf '\n== build tests/test_%s.mojo ==\n' "$test_name"
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5da"
    printf '== run test_%s_12c5da ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5da"
done
...
printf '%s\n' 'stage12c5da prompt profile explicit copy complete'
