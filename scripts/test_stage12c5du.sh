...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5dt.sh" -- "$@"
fi

printf '\n== prompt execution historical effect plan ==\n'
for test_name in prompt_execution prompt_interaction prompt_legacy_echo prompt_table_execution legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5du"
    printf '== run test_%s_12c5du ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5du"
done
...
printf '%s\n' 'stage12c5du prompt execution historical effect plan complete'
