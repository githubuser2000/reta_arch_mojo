...
printf '\n== prompt execution one-shot result import coverage ==\n'
for test_name in prompt_execution prompt_runtime prompt_interaction prompt_legacy_echo prompt_table_execution legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5fa"
    printf '== run test_%s_12c5fa ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5fa"
done

"$ROOT/scripts/run_pytest.sh" -q \
    tests/test_stage12c5fa_source.py \
...
printf '%s\n' 'stage12c5fa prompt execution one-shot result import coverage complete'
