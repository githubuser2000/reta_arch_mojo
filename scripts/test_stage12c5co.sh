...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cn.sh" -- "$@"
fi

printf '\n== external reta child argument ownership ==\n'
for test_name in prompt_interaction legacy_reta_prompt table_adapters; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5co"
    printf '== run test_%s_12c5co ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5co"
done
...
printf '%s\n' 'stage12c5co external reta child argument ownership complete'
