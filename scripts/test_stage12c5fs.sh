...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5fr.sh" -- "$@"
fi

printf '\n== public core launchers ==\n'
"$ROOT/scripts/build_core_shared.sh" --dry-run -- "$@"
"$ROOT/scripts/build_shared_library_targets.sh" --dry-run -- "$@"

# Explicit source names kept for guards: tests/test_shared_library_architecture.mojo test_shared_library_architecture_12c5fs
for test_name in shared_library_architecture; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5fs"
    printf '== run test_%s_12c5fs ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5fs"
done
...
printf '%s\n' 'stage12c5fs public core launchers complete'
