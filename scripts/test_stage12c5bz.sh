...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5by.sh" -- "$@"
fi

printf '\n== stored prompt default enter ownership ==\n'
for test_name in prompt_interaction legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5bz"
    printf '== run test_%s_12c5bz ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5bz"
done
...
printf '%s\n' 'stage12c5bz stored prompt default enter ownership complete'
