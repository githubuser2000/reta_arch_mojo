...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5bs.sh" -- "$@"
fi

printf '\n== frozen informational companion-effect order ==\n'
...
"$ROOT/scripts/run_pytest.sh" -q \
    ...
    tests/test_prompt_table_execution_regressions_source.py \
...
printf '%s\n' \
    'stage12c5bt informational prompt companion effects complete'
