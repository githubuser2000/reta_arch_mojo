#!/usr/bin/env python3
"""Batch-print reference completions while bootstrapping one language once."""
...
if len(sys.argv) != 3:
    raise SystemExit("usage: prompt_completion_reference_batch.py LANGUAGE CONTEXTS.tsv")
language, contexts_path = sys.argv[1:]
...
runtime = bootstrap_completion_runtime(architecture=architecture, i18n=i18n, force_rebuild=True)
event = CompleteEvent(completion_requested=True)


def new_completer():
    return NestedCompleter(
        {command: None for command in runtime.start_commands(include_numeric_shortcuts=True)},
        {},
        ComplSitua.retaAnfang,
        "",
        {
            **{"reta": ComplSitua.retaAnfang},
            **{command: ComplSitua.befehleNichtReta for command in runtime.befehle2},
        },
        completion_runtime=runtime,
        i18n=i18n,
    )

for raw in Path(contexts_path).read_text(encoding="utf-8").splitlines():
