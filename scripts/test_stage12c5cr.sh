...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cq.sh" -- "$@"
fi

printf '\n== legacy bridge payload entrypoints ==\n'
for test_name in legacy_mojo_bridge prompt_interaction legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cr"
    printf '== run test_%s_12c5cr ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cr"
done
...
"$ROOT/scripts/run_pytest.sh" -q \
    ...
    tests/test_stage12c5cl_source.py \
    tests/test_stage12c5ck_source.py \
    tests/test_stage12c5cj_source.py \
...
printf '%s\n' 'stage12c5cr legacy bridge payload entrypoints complete'
