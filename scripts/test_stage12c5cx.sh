...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cw.sh" -- "$@"
fi

printf '\n== external raw payload helper localization ==\n'
for test_name in legacy_mojo_bridge prompt_external_commands prompt_interaction; do
    printf '\n== build tests/test_%s.mojo ==\n' "$test_name"
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cx"
    printf '== run test_%s_12c5cx ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cx"
done
...
"$ROOT/scripts/run_pytest.sh" -q \
    ...
    tests/test_stage12c5ct_source.py \
    tests/test_stage12c5cr_source.py \
    tests/test_stage12c5cm_source.py \
...
printf '%s\n' 'stage12c5cx external raw payload helper localization complete'
