...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5de.sh" -- "$@"
fi

printf '\n== fallback runtime argv builder owner ==\n'
for test_name in prompt_interaction legacy_reta_prompt legacy_mojo_bridge prompt_external_commands; do
    printf '\n== build tests/test_%s.mojo ==\n' "$test_name"
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5df"
    printf '== run test_%s_12c5df ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5df"
done
...
printf '%s\n' 'stage12c5df fallback runtime argv builder owner complete'
