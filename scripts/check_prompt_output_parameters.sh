...
mkdir -p "$TEST_DIR"
BINARY="$TEST_DIR/prompt-output-parameter-probe"
if mojo_has_thread_option "$@"; then
    "$MOJO" build --no-optimization -I src \
        tests/prompt_output_parameter_probe.mojo "$@" -o "$BINARY"
else
    "$MOJO" build --no-optimization -j 4 -I src \
        tests/prompt_output_parameter_probe.mojo -o "$BINARY"
fi
PYTHONHASHSEED=0 PYTHONDONTWRITEBYTECODE=1 \
    "$TEST_PYTHON" scripts/check_prompt_output_parameters.py "$BINARY"
