...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5dd.sh" -- "$@"
fi

printf '\n== fallback merged argument owner ==\n'
for test_name in prompt_interaction legacy_reta_prompt legacy_mojo_bridge prompt_external_commands; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5de"
    printf '== run test_%s_12c5de ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5de"
done
...
printf '%s\n' 'stage12c5de fallback merged argument owner complete'
