...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5dl.sh" -- "$@"
fi

printf '\n== prompt reaction dispatch owner ==\n'
for test_name in prompt_interaction prompt_external_commands legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5dm"
    printf '== run test_%s_12c5dm ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5dm"
done
...
printf '%s\n' 'stage12c5dm prompt reaction dispatch owner complete'
