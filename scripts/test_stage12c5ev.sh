...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5eu.sh" -- "$@"
fi

printf '\n== prompt process native reta child result owner ==\n'
for test_name in prompt_execution prompt_runtime prompt_interaction prompt_legacy_echo prompt_table_execution legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5ev"
    printf '== run test_%s_12c5ev ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5ev"
done
...
printf '%s\n' 'stage12c5ev prompt process native reta child result owner complete'
