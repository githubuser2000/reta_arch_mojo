...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5fj.sh" -- "$@"
fi

printf '\n== prompt execution one-shot probe pipeline state owner ==\n'
for test_name in prompt_execution prompt_runtime prompt_interaction prompt_legacy_echo prompt_table_execution legacy_mojo_bridge legacy_reta_prompt; do
    ...
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5fk"
    printf '== run test_%s_12c5fk ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5fk"
done

"$ROOT/scripts/run_pytest.sh" -q \
    tests/test_stage12c5fk_source.py \
    tests/test_stage12c5fj_source.py \
    tests/test_stage12c5fi_source.py \
    tests/test_stage12c5fh_source.py \
    tests/test_stage12c5fg_source.py \
    tests/test_stage12c5ff_source.py \
    tests/test_stage12c5fe_source.py \
    tests/test_stage12c5fd_source.py \
    tests/test_stage12c5fc_source.py \
    tests/test_stage12c5fb_source.py \
    tests/test_stage12c5ez_source.py \
    tests/test_stage12c5ey_source.py \
...
printf '%s\n' 'stage12c5fk prompt execution one-shot probe pipeline state owner complete'
