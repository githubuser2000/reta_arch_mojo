...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5bn.sh" -- "$@"
fi

printf '\n== canonical lowercase emotion option in native fraction probe ==\n'
"$ROOT/scripts/check_prompt_true_fraction_multiples.sh" -- "$@"

"$ROOT/scripts/run_pytest.sh" -q \
    tests/test_stage12c5bo_source.py \
...
printf '%s\n' \
    'stage12c5bo canonical native emotion option contract complete'
