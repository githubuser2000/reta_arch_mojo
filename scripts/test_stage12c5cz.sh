...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5cy.sh" -- "$@"
fi

printf '\n== shared reta child argv owner ==\n'
for test_name in prompt_external_commands legacy_mojo_bridge; do
    printf '\n== build tests/test_%s.mojo ==\n' "$test_name"
    "$MOJO" build -I src -I tests "tests/test_${test_name}.mojo" "$@" \
        -o "$TARGET/test_${test_name}_12c5cz"
    printf '== run test_%s_12c5cz ==\n' "$test_name"
    "$ROOT/tools/wrappers/mojo-runtime-exec" "$TARGET/test_${test_name}_12c5cz"
done
...
printf '%s\n' 'stage12c5cz shared reta child argv owner complete'
