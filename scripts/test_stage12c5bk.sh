...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5bj.sh" -- "$@"
fi

printf '\n== hermetic native representative command parity ==\n'
...
printf '\n== compile/run classic integer guard and true-fraction probe ==\n'
"$ROOT/scripts/check_prompt_true_fraction_multiples.sh" -- "$@"

printf '\n== compile/run complete table runtime owner ==\n'
...
"$ROOT/scripts/run_pytest.sh" -q \
    ...
    tests/test_incremental_test_build.py \
...
printf '%s\n' \
    'stage12c5bk hermetic parity, incremental test builds and fraction guards complete'
