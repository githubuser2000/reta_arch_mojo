...
if [ "${RETA_STAGE_SKIP_PREVIOUS:-0}" != 1 ]; then
    "$ROOT/scripts/test_stage12c5dv.sh" -- "$@"
fi

printf '\n== prompt execution compact announcement regression fix ==\n'
...
printf '%s\n' 'stage12c5dv prompt execution compact announcement regression fix complete'
