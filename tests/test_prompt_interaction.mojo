...
def test_contract_snapshot() raises:
    ...
    assert_equal(len(process_snapshot), 24)
    ...
    assert_equal(process_snapshot[9], "one_shot_external_execution=native-prompt-process-one-shot-execution-boundary")
    assert_equal(process_snapshot[10], "one_shot_external_boundary=native-prompt-process-probe-boundary")
    assert_equal(process_snapshot[11], "external_reta_child=native-prompt-reta-child-argv")
    assert_equal(process_snapshot[12], "external_raw_line=eliminated-from-external-process-plan")
    assert_equal(process_snapshot[13], "external_shell_arguments=native-prompt-shell-argv-plan")
    assert_equal(process_snapshot[14], "external_python_math_arguments=native-prompt-python-math-argv-plan")
    assert_equal(process_snapshot[15], "external_command_arguments=runtime-owned-command-argv-builders")
    assert_equal(process_snapshot[16], "fallback_process_dispatch=native-interaction-argv-plan")
    assert_equal(process_snapshot[17], "fallback_process_execution=native-prompt-fallback-execution-boundary")
    assert_equal(process_snapshot[18], "fallback_process_handled=native-explicit-fallback-effect-flag")
    assert_equal(process_snapshot[19], "fallback_process_flags=native-explicit-fallback-run-flag")
    assert_equal(process_snapshot[20], "fallback_process_arguments=native-merged-fallback-argv")
    assert_equal(process_snapshot[21], "fallback_runtime_arguments=runtime-owned-argv-builder")
    assert_equal(process_snapshot[22], "fallback_shell_split=runtime-owned-argv-tokenizer")
    assert_equal(process_snapshot[23], "process_adapter=argv-execution-only")
