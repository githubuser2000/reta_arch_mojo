...
def test_one_shot_external_result_is_planned_by_process_execution_owner() raises:
    ...
    assert_false(shell_result.continue_native_probe)
    ...
    assert_false(accepted_result.continue_native_probe)
    ...
    assert_true(unhandled_result.continue_native_probe)
    assert_false(unhandled_result.reta_native_handled)


def test_native_reta_child_result_is_planned_by_process_execution_owner() raises:
    var startup = plan_prompt_native_reta_child_result(True, False)
    assert_true(startup.handled)
    assert_true(startup.print_startup_output)
    assert_false(startup.print_native_reta_output)

    var native_table = plan_prompt_native_reta_child_result(False, True)
    assert_true(native_table.handled)
    assert_false(native_table.print_startup_output)
    assert_true(native_table.print_native_reta_output)

    var rejected = plan_prompt_native_reta_child_result(False, False)
    assert_false(rejected.handled)
    assert_false(rejected.print_startup_output)
    assert_false(rejected.print_native_reta_output)
...
def test_contract_snapshot() raises:
    ...
    assert_equal(len(process_snapshot), 33)
    ...
    assert_equal(process_snapshot[14], "native_reta_child_result=native-prompt-reta-child-result-boundary")
    assert_equal(process_snapshot[15], "external_reta_child=native-prompt-reta-child-argv")
    assert_equal(process_snapshot[16], "external_raw_line=eliminated-from-external-process-plan")
    assert_equal(process_snapshot[17], "external_shell_arguments=native-prompt-shell-argv-plan")
    assert_equal(process_snapshot[18], "external_python_math_arguments=native-prompt-python-math-argv-plan")
    assert_equal(process_snapshot[19], "external_command_arguments=runtime-owned-command-argv-builders")
    assert_equal(process_snapshot[20], "compatibility_fallback_process_execution=native-prompt-compatibility-fallback-execution-boundary")
    assert_equal(process_snapshot[21], "compatibility_fallback_process_result=native-prompt-compatibility-fallback-result-boundary")
    assert_equal(process_snapshot[22], "residual_fallback_process_execution=native-prompt-residual-fallback-execution-boundary")
    assert_equal(process_snapshot[23], "residual_fallback_process_result=native-prompt-residual-fallback-result-boundary")
    assert_equal(process_snapshot[24], "fallback_process_dispatch=native-interaction-argv-plan")
    assert_equal(process_snapshot[25], "fallback_process_execution=native-prompt-fallback-execution-boundary")
    assert_equal(process_snapshot[26], "fallback_process_result=native-prompt-fallback-result-boundary")
    assert_equal(process_snapshot[27], "fallback_process_handled=native-explicit-fallback-effect-flag")
    assert_equal(process_snapshot[28], "fallback_process_flags=native-explicit-fallback-run-flag")
    assert_equal(process_snapshot[29], "fallback_process_arguments=native-merged-fallback-argv")
    assert_equal(process_snapshot[30], "fallback_runtime_arguments=runtime-owned-argv-builder")
    assert_equal(process_snapshot[31], "fallback_shell_split=runtime-owned-argv-tokenizer")
    assert_equal(process_snapshot[32], "process_adapter=argv-execution-only")
