...
def test_current_stage_points_to_dr_or_later() -> None:
    ...
    assert "test_stage12c5dr.sh" in current or "test_stage12c5ds.sh" in current
...
def test_prompt_execution_owns_table_and_mulpri_ownership_decision() -> None:
    ...
    assert controller.count("var ownership = plan_prompt_execution_table_ownership(") == 2
...
def test_table_ownership_plan_preserves_atomic_fallback_shape() -> None:
    ...
    assert "if ownership.fallback_required:" in controller
    assert "ownership.table_plan" in controller
    assert "ownership.owns_table or ownership.owns_mulpri" in controller
