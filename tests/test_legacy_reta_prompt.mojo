...
def test_historical_input_and_start_delegate_to_native_controller() raises:
    ...
    assert_equal(scope[27], "native_reta_child_result=native-prompt-reta-child-result-boundary")
    assert_equal(scope[33], "compatibility_fallback_process_execution=native-prompt-compatibility-fallback-execution-boundary")
    assert_equal(scope[34], "compatibility_fallback_process_result=native-prompt-compatibility-fallback-result-boundary")
    assert_equal(scope[35], "residual_fallback_process_execution=native-prompt-residual-fallback-execution-boundary")
    assert_equal(scope[36], "residual_fallback_process_result=native-prompt-residual-fallback-result-boundary")
    assert_equal(scope[39], "fallback_process_result=native-prompt-fallback-result-boundary")
    ...
    assert_equal(scope[45], "external_dispatch_owner=prompt-execution-process-plan")
