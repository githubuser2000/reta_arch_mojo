...
def main() raises:
    ...
    var serial = make_parallel_config("off", 2, 2, 1, "fork", "unit")
    ...
    checks += 1
    assert_true(len(religion.rows) == len(religion_reference), "religion row count")
    checks += 1
    for row_index in range(len(religion.rows)):
        assert_true(
            religion.rows[row_index].index == religion_reference[row_index].index,
    ...
    var fallback = decode_religion_rows_in_processes(religion_rows, "html", serial)
    assert_true(fallback.stats.mode == "serial", "religion serial fallback")
    checks += 1
    for row_index in range(len(fallback.rows)):
        assert_true(
            fallback.rows[row_index].index == religion_reference[row_index].index,
    ...
    print("parallel row process tests:", checks, "/", checks)
