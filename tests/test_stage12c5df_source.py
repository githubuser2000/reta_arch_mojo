...
def test_mojo_snapshot_tracks_runtime_fallback_argv_owner() -> None:
    ...
    assert ("assert_equal(len(snapshot), 33)" in test or "assert_equal(len(snapshot), 34)" in test or "assert_equal(len(snapshot), 35)" in test or "assert_equal(len(snapshot), 36)" in test)
