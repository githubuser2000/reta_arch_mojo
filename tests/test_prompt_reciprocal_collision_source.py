...
def test_runtime_contract_requires_thirteen_native_invocations() -> None:
    mojo = MOJO_TEST.read_text(encoding="utf-8")
    checker = CHECKER.read_text(encoding="utf-8")
    probe = PROBE.read_text(encoding="utf-8")
    command = "universum v1/4,-1/8,2/3"
    assert f'var reciprocal_collision = _plan("{command}")' in mojo
    ...
    assert "expected_collision_rows" in checker
