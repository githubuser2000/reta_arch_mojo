...
def main() raises:
    var checks = 0
    var config = make_parallel_config("processes", 2, 1, 1, "fork", "unit")
    var table = CsvTable(
    ...
    var selected = select_columns_in_processes(table, columns, config)
    assert_true(selected.stats.mode == "processes", "select mode")
    checks += 1
    ...
    for row in range(len(selected_serial.rows)):
        for column in range(len(selected_serial.rows[row])):
            assert_true(selected.table.rows[row][column] == selected_serial.rows[row][column], "select parity")
            checks += 1
    ...
    var widths = max_cell_text_len_in_processes(fragments, fragment_indexes, config)
    assert_true(widths.stats.mode == "processes", "width mode")
    checks += 1
    assert_true(len(widths.widths) == len(width_serial), "width count")
    checks += 1
    for index in range(len(width_serial)):
        assert_true(widths.widths[index].column == width_serial[index].column, "width column")
        checks += 1
        assert_true(widths.widths[index].width == width_serial[index].width, "width value")
        checks += 1

    var positive0 = Set[Int](); positive0.add(1); positive0.add(2); positive0.add(3)
    var negative0 = Set[Int](); negative0.add(2)
    var positive1 = Set[Int](); positive1.add(7); positive1.add(9)
    var negative1 = Set[Int](); negative1.add(9)
    ...
    var normalized = normalize_column_buckets_in_processes(buckets, config)
    assert_true(normalized.stats.mode == "processes", "bucket mode")
    checks += 1
    ...
    checks += 1
    assert_true(2 not in normalized.buckets[0].positive, "bucket subtracts negative")
    checks += 1
    assert_true(len(normalized.buckets[0].negative) == 0, "bucket clears negative")
    checks += 1
    ...
    var joined = prepare_kombi_join_tables_in_processes(selections, table, config)
    assert_true(joined.stats.mode == "processes", "join mode")
    checks += 1
    assert_true(len(joined.selections) == 2 and len(joined.tables) == 2, "join count")
    checks += 1
