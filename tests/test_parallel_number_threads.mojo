...
def main() raises:
    ...
    assert_true(moon.stats.mode == "threads", "moon thread mode")
    checks += 1
    ...
    for index in range(len(moon.values)):
        assert_true(
            moon.values[index].number == moon_reference[index].number,
            "moon order",
        )
        checks += 1
        assert_true(
    ...
    assert_true(factors.stats.mode == "threads", "factor thread mode")
    checks += 1
    ...
    assert_true(filtered.stats.mode == "threads", "filter thread mode")
    checks += 1
    assert_true(len(filtered.values) == len(filtered_reference), "filter count")
    checks += 1
    for index in range(len(filtered.values)):
        assert_true(
            filtered.values[index] == filtered_reference[index], "filter parity"
        )
        checks += 1
    assert_true(
        filtered.values[0] == 6
    ...
    print("parallel number thread tests:", checks, "/", checks)
