...
def test_shared_library_plan_matches_requested_split() raises:
    ...
    assert_equal(plan.core_library, "libreta_core_mojo")
    assert_equal(plan.prompt_library, "libreta_prompt_mojo")
    assert_equal(plan.prompt_interactive_library, "libreta_prompt_interactive_mojo")
...
def test_rpb_is_one_shot_without_interactive_input_library() raises:
    ...
    for index in range(len(starters)):
        if starters[index].starter_name == "rpb":
            ...
            assert_equal(starters[index].libraries[0], "libreta_prompt_mojo")
            assert_equal(starters[index].libraries[1], "libreta_core_mojo")
    assert_true(found)
...
def test_interactive_prompt_starters_share_interactive_library() raises:
    ...
    for index in range(len(starters)):
        if starters[index].interactive:
            interactive_count += 1
            assert_equal(starters[index].libraries[0], "libreta_prompt_interactive_mojo")
            assert_equal(starters[index].libraries[1], "libreta_prompt_mojo")
            assert_equal(starters[index].libraries[2], "libreta_core_mojo")
...
def test_core_library_is_shared_by_reta_prompts_and_grundstrukhtml() raises:
    ...
    for index in range(len(targets)):
        if targets[index].logical_name == "libreta_core_mojo":
            found_core = True
            assert_equal(targets[index].elf_name, "libreta_core_mojo.so")
            assert_equal(targets[index].dll_name, "libreta_core_mojo.dll")
