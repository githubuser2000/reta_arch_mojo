...
def test_default_commit_and_public_entrypoint_are_current() -> None:
    ...
    status_log = (ROOT / "PROJECT_STATUS_LOG.md").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    assert 'COMMIT_MESSAGE=${1:-12c5bk}' in do_script
    assert "PROJECT_STATUS_LOG.md" in readme
    assert "STAGE12C5BG_DETERMINISTIC_COMMAND_PARITY_INTEGER_FRACTION_AXES.md" in status_log
