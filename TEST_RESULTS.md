# Test Results – Stage 12c5cq

## Compilerfreie lokale Prüfung

- Neuer compilerfreier Gate deckt `tests/test_stage12c5cq_source.py`, die vorherigen externen Prompt-Stageverträge, den Prompt-Interaktionsbesitzer, historische Begleiteffekte, Tabellenadapter-Parität, Ledger, Metrik und Archivvertrag ab: **106/106** fokussierte Source-Verträge grün.
...
- Mojo-Zeilen in `src/`: **64.516**; in `src/reta_mojo/`: **59.915**.
...
- Neuer compilerfreier Gate deckt `tests/test_stage12c5cm_source.py`, die vorherigen 12c5c*-Stageverträge, den Prompt-Interaktionsbesitzer, historische Begleiteffekte, Tabellenadapter-Parität, Ledger, Metrik und Archivvertrag ab: **124/124** fokussierte Source-Verträge grün.
- Defektledger: **169** bekannte Defekte, **19** spätere Python-Aufgaben, konsistent.
- Portierungsmetrik: **89/92 = 96,7 %** vollständig nativ/generiert, **92/92 = 100,0 %** mindestens teilweise portiert.
- Mojo-Zeilen in `src/`: **64.503**; in `src/reta_mojo/`: **59.900**.
- Manifest nach Aktualisierung: **1.773 Dateien**, **114 Symlinks**.
- Mojo-Kompilierung wurde in dieser Umgebung nicht ausgeführt; der Benutzer kompiliert die nativen Ziele lokal.

# Test Results – Stage 12c5cl

## Compilerfreie lokale Prüfung

- Neuer compilerfreier Gate deckt `tests/test_stage12c5cl_source.py`, die vorherigen 12c5c*-Stageverträge, den Prompt-Interaktionsbesitzer, historische Begleiteffekte, Tabellenadapter-Parität, Ledger, Metrik und Archivvertrag ab: **118/118** fokussierte Source-Verträge grün.
- Defektledger: **169** bekannte Defekte, **19** spätere Python-Aufgaben, konsistent.
- Portierungsmetrik: **89/92 = 96,7 %** vollständig nativ/generiert, **92/92 = 100,0 %** mindestens teilweise portiert.
- Mojo-Zeilen in `src/`: **64.443**; in `src/reta_mojo/`: **59.840**.
- Manifest nach Aktualisierung: **1.770 Dateien**, **114 Symlinks**.
- Mojo-Kompilierung wurde in dieser Umgebung nicht ausgeführt; der Benutzer kompiliert die nativen Ziele lokal.
...
- Benutzerlogs zu 12c5cc/12c5cd: breite Mojo-Auswahl erfolgreich abgeschlossen; `test_table_adapters` grün; fokussierter 12c5cd-Stage-Lauf mit 18/18 Prompt-Interaktion und grünen Legacy-/Table-Adapter-Zielen.
- Neuer compilerfreier Gate deckt `tests/test_stage12c5cf_source.py`, den Prompt-Interaktionsbesitzer, historische Begleiteffekte, Tabellenadapter-Parität, Ledger, Metrik und Archivvertrag ab: 87/87 fokussierte Source-Verträge grün.
- Defektledger: 169 Einträge, 19 spätere Python-Aufgaben; Portierung: 89/92 vollständig, 92/92 mindestens teilweise; `src/`: 64.284 Mojo-Zeilen, davon 59.619 in `src/reta_mojo/`.
- Die tatsächliche Mojo-Kompilierung bleibt der Benutzerlauf `RETA_STAGE_SKIP_PREVIOUS=1 scripts/test_stage12c5cf.sh -- -j 8`.
...
- Vollständiger nativer Build 12c5ca: erfolgreich.
- `test_prompt_interaction.mojo`: **16/16** bestanden.
- `test_legacy_reta_prompt.mojo`: **4/4** bestanden.
- Fokussierte 12c5ca-Sourcegruppe: **70/70** bestanden.

## Compilerfreie Prüfung in der Erstellungsumgebung

- Fokussierte Stage-/Source-/Ledger-/Matrix-/Archivgruppe: **75/75** bestanden.
- Defektledger: **169** Einträge, **19** spätere Python-Aufgaben.
- Portierung: **89/92** vollständig, **92/92** mindestens teilweise.
- Mojo-Zeilen in `src/`: **64.193**, davon **59.522** in `src/reta_mojo/`.
- Quellmanifest und frisch entpacktes Archiv werden vor Auslieferung erneut geprüft.
...
## Stage 12c5cb – vorbereitete Prüfungen

- Benutzerlauf 12c5ca: `test_prompt_interaction.mojo` **16/16**, `test_legacy_reta_prompt.mojo` **4/4**, Sourcegruppe **70/70**.
