...
| `reta_architecture/prompt_execution.py` | 2516 | 24 | 1 | 3 | weitgehend nativ | `src/reta_mojo/prompt_* + prompt_main.mojo` | zentrale Kurzsprache, Tabellenplanung, numerische Fachbefehle, One-shots sowie `shell`/`python`/`math` nativ; gemischte `vielfache + teiler + 1/n`-Pläne bytegenau nativ; Restfallbacktransport und nicht-native `reta`-Zeilen direkt aus Mojo gestartet; echte v-n/m-Restalgorithmen und TTY-Editor offen |
