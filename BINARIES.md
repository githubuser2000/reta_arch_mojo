...
| `rp`, `rpl`, `rpb`, `rpe`, `retaPrompt*` | `target/bin/reta-prompt-native` | explizite native One-shots und besessene Tabellen laufen ohne Python-/`reta-native`-Kindprozess; kompakte Echoformen, echte `v n/m`-Vielfache und kollidierende Legacy-Sonderoperationen bleiben an der Bridge |
