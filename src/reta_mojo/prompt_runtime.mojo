...
def split_prompt_words(text: String) -> List[String]:
    """Split prompt input into words for command dispatch.

    This deliberately performs only whitespace splitting. The untouched raw line
    is retained and passed to the process bridge, which performs shell-compatible
    quote handling where exact argv reconstruction matters.
    """
    var result = List[String]()
    _append_words(text, result)
    return result^
...
def parse_prompt_startup(
    ...
    for index in range(len(arguments)):
        ...
        if argument == "-befehl":
            ...
            show_help = True
        elif argument == "-language=english":
